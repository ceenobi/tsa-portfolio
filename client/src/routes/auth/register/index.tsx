...
export default function Register() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: res => {
      if (import.meta.env.DEV) console.log('suss', res)
      if (res.success) {
        toast.success('Account created successfully')
        navigate(`/auth/verify-email?email=${res?.body?.user?.email}`)
      }
    },
    onError: err => {
  ...
  return (
    <div className="bg-white w-full max-w-170 mx-auto border space-y-8 py-10 px-8 rounded-lg">
      <Logo />
      <div className="mt-4 space-y-1">
        <h1 className="text-xl sm:text-2xl md:text-[28px] font=semibold">Create an Admin Account</h1>
        <p className="text-sm md:text-lg text-mainGray">Register to manage the Academy Portfolio Archive</p>
      </div>
